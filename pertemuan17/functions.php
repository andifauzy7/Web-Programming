...
function register($data){
	...
	$pass = $data["password"];
	$pass_temp = $data["password2"];

	$double_user = query("SELECT username FROM user WHERE username='$username';");
	if(!empty($double_user)){
		// Jika Variabel Double_user tidak kosong.
		echo '
			<script>
	...
	// Jika Variable Kosong (Tidak ada duplikat)
	if($pass !== $pass_temp){
		echo '
			<script>
	...
	// Proses Enkripsi Password untuk Keamanan
	$pass = password_hash($pass, PASSWORD_DEFAULT);
	$username = strtolower($username);

	mysqli_query($string_conn, "INSERT INTO user VALUES ('','$username','$pass');");
	return mysqli_affected_rows($string_conn);
}
