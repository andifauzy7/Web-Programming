<?php 
	...
	// Mencari jumlah dari database.
	$squad 				= query("SELECT * FROM pemain");
	$jumlahdata 		= count($squad);
	$perhalaman 		= 4; // DATA PER HALAMAN YAITU 3 RECORD.
	$jumlah_page 		= ceil($jumlahdata / $perhalaman);
	$halaman_active 	= ( isset($_GET["page"]) ) ? $halaman_active = $_GET["page"] : 1;

	$start 				= $halaman_active * $perhalaman - $perhalaman;
	// LIMIT parameter pertama (DARI DATA KE BERAPA) parameter kedua (BERAPA BANYAK)
	$squad 				= query("SELECT * FROM pemain LIMIT $start, $perhalaman;");
...
<body>
	...
	<div class="jumbotron jumbotron-fluid" style="background-color: #2c3e50; margin: 0;">
		<div class="container">
			...
			<div class="row">
				<div class="col-6">
					<a href="logout.php"><button type="button" class="btn btn-outline-danger" name="submit">Logout</button></a>
			...
			<div class="row">
				<div class="col-12">
					<br>
						<?php if($halaman_active > 1) : ?>
							<a href="?page=<?=$halaman_active-1;?>"> < </a>
						<?php endif; ?>

						<?php for ($i=1; $i<=$jumlah_page; $i++) : ?>
							<?php if($i == $halaman_active) : ?>
						...
						<?php if($halaman_active < $jumlah_page) : ?>
							<a href="?page=<?=$halaman_active+1;?>"> > </a>
						<?php endif; ?>
				</div>
			</div>
